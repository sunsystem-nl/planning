...
export const GetAllPlanningBediening = () => {
	...
	const [selectedColumns, setSelectedColumns] = useState(JSON.parse(localStorage.getItem('columnsAllPlanningBediening')) || columns)
	...
	const onColumnToggle = (event) => {
		...
		localStorage.setItem('columnsAllPlanningBediening', JSON.stringify(selectedColumns))
	}
	...
	const fetchResults = useCallback(async () => {
		try {
			const res = await axios.get(`${process.env.REACT_APP_API_URL}/orders/${selectedYear}/${selectedWeek}/bediening`, {
				headers: {
					'Accept': 'application/json',
					'Content-Type': 'application/json',
					'Authorization': `Bearer ${token}`,
	...
	return (
		...
		>
			<PageTitle title={`Bediening planning voor ${selectedYear} week ${selectedWeek}.`} />
			<Messages ref={message} id={'message'} />
			<iframe id="ifmcontentstoprint" style={{ height: '0px', width: '0px', position: 'absolute', fontSize: '30rem' }} title='getallplanningbediening'></iframe>
