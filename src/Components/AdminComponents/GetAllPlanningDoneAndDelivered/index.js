...
export const GetAllPlanningDoneAndDelivered = () => {
	...
	const [selectedColumns, setSelectedColumns] = useState(JSON.parse(localStorage.getItem('columnsDoneAndDelivered')) || columns)
	...
	const onColumnToggle = (event) => {
		...
		localStorage.setItem('columnsDoneAndDelivered', JSON.stringify(selectedColumns))
	}
	...
	const fetchResults = useCallback(async () => {
		try {
			const res = await axios.get(`${process.env.REACT_APP_API_URL}/orders/${selectedYear}/gereed-en-afgehaald`, {
				headers: {
					'Accept': 'application/json',
					'Content-Type': 'application/json',
					'Authorization': `Bearer ${token}`,
	...
	return (
		...
		>
			<PageTitle title={`Opdrachten klaar in ${selectedYear} en afgehaald.`} />
			<Messages ref={message} id={'message'} />
			<iframe id="ifmcontentstoprint" style={{ height: '0px', width: '0px', position: 'absolute', fontSize: '30rem' }} title="getalldoneanddelivered"></iframe>
