...
export const GetAllPlanningPantsers = () => {
	...
	const [selectedColumns, setSelectedColumns] = useState(JSON.parse(localStorage.getItem('columnsAllPlanningPantser')) || columns)
	...
	const onColumnToggle = (event) => {
		...
		localStorage.setItem('columnsAllPlanningPantser', JSON.stringify(selectedColumns))
	}
	...
	const referentieBodyTemplate = (rowData) => {
		return <Button className={rowData.urgent === true ? "p-button-success text-white" : "p-button-text text-dark"} label={rowData.referentie} />
	}
	...
	const fetchResults = useCallback(async () => {
		try {
			const res = await axios.get(`${process.env.REACT_APP_API_URL}/orders/${selectedYear}/${selectedWeek}/pantsers`, {
				headers: {
					'Accept': 'application/json',
					'Content-Type': 'application/json',
					'Authorization': `Bearer ${token}`,
	...
	return (
		...
		>
			<PageTitle title={`Pantser planning voor ${selectedYear} week ${selectedWeek}.`} />
			<Messages ref={message} id={'message'} />
			<iframe id="ifmcontentstoprint" style={{ height: '0px', width: '0px', position: 'absolute', fontSize: '30rem' }} title='getallplanningpantsers'></iframe>
