...
export const GetAllPlanningThisWeek = () => {
	...
	const [selectedColumns, setSelectedColumns] = useState(JSON.parse(localStorage.getItem('columnsThisWeek')) || columns)
	...
	const onColumnToggle = (event) => {
		...
		localStorage.setItem('columnsThisWeek', JSON.stringify(selectedColumns))
	}
	...
	const fetchResults = useCallback(async () => {
		try {
			const res = await axios.get(`${process.env.REACT_APP_API_URL}/orders/${selectedYear}/${currentWeek}`, {
				headers: {
					'Accept': 'application/json',
					'Content-Type': 'application/json',
					'Authorization': `Bearer ${token}`,
	...
	return (
		...
		>
			<PageTitle title={`Planning voor ${selectedYear} in week ${currentWeek}.`} />
			<Messages ref={message} id={'message'} />
			<iframe id="ifmcontentstoprint" style={{ height: '0px', width: '0px', position: 'absolute', fontSize: '30rem' }} title='getallplanningthisweek'></iframe>
